	...
	btnFunc : {
		...
		btnSubmitEx : function (param){
			...
			$.post(url,$.param(data_obj),function(Json){
				...
				ret_state = DataTransDecode(Json.ret_state);
				// alert(Json);
				// alert(Json.ret_state[0].img_url);
				// alert(ret_code);
				//判断返回标志位
				if(ret_code == 0)
				{
					...
					// alert(ret_state);
					// 数据操作成功
					// var httpRegUrl = new RegExp();  // 带http  ftp 等等 
					// httpRegUrl.compile("^[A-Za-z]+://[A-Za-z0-9-_]+\\.[A-Za-z0-9-_%&\?\/.=]+$");//jihua.cnblogs.com
					var RegUrl = new RegExp();
					RegUrl.compile("[A-Za-z0-9-_]+\\.[A-Za-z0-9-_%&\?\/.=]+$");  // 正则表达式检测返回的ret_state  是否是url
					// 设置回调
					...
					}else if(RegUrl.test(ret_state)){
						top.location.href=ret_state;
					}else{// 当ret_state 不为空时 也不是url的时候  
						if(typeof(HomeCtrlElAction.OncallBackSuccess) == "function"){ // 判断传递的参数是否为方法
							HomeCtrlElAction.OncallBackSuccess(Json.ret_state);
						}
					}
				}
				else
