	...
	initCtrl:function(i_options){
		...
		this_obj.sys_opt = {
			...
			page_area				: {
				...
				//页面的高度
				page_height				: 0,
				//菜单栏是否展开,false-否(收起),true-是(展开),右划为打开
				menu_list_unfold_if		: false,
				//比例图是否展开,向上滑动则为展开
				ratio_digm_unfold_if	: false,
			},
			//战略地图
			fight_map_data			: {
			...
			ring_ratio_dgm 			: {
				...
				//小屏幕限制高度
				rdr_min_screen_limit		: 227,
				//区域高度
				rdr_sub_area_1_height		: 30,
				rdr_sub_area_2_height		: 0,
				rdr_sub_area_3_height		: 30,
				rdr_sub_area_4_height		: 0,
				rdr_sub_area_5_height		: 65,

				//区域4原始高度
				rdr_sub_area_4_org_height		: 248,
			...
			help_poo_intro_area		:{
				//区域的高度
				hp_sub_area_1_height	: 50,
				hp_sub_area_2_height	: 0,
				hp_sub_area_3_height	: 65,
		...
		this_obj.func_set.sys_init_func = {
			...
			initRatioRingDmgArea : function(){
				...
				//区域1高度
				//this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_1_height	= 
				//区域2高度
				this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_2_height	= this_obj.sys_opt.rd_ring_con_height;
				//区域3高度
				//区域4的高度:总高度-区域1高度-区域2高度-区域3高度-区域5高度
				this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height = this_obj.sys_opt.ratio_dgm_con_height;
				this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height -= this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_1_height;
				this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height -= this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_2_height;
				this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height -= this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_3_height;
				this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height -= this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_5_height;
				//alert(this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height);
				...
				this_obj.sys_opt.ring_ratio_dgm.ratio_dgm_con_top_offset 	= this_obj.sys_opt.dis_area_height;
			...
			initHelpPoorIntroArea	: function(){
				...
				//计算区域2的高度:总高度-区域1高度-区域3高度
				this_obj.sys_opt.help_poo_intro_area.hp_sub_area_2_height = this_obj.sys_opt.page_area.page_height;
				this_obj.sys_opt.help_poo_intro_area.hp_sub_area_2_height -= this_obj.sys_opt.help_poo_intro_area.hp_sub_area_1_height;
				this_obj.sys_opt.help_poo_intro_area.hp_sub_area_2_height -= this_obj.sys_opt.help_poo_intro_area.hp_sub_area_3_height;
			},
		};//系统初始化函数集
		...
		this_obj.func_set.html_dom_func = {
			...
			createCanvasAreaEl : function(){
				...
				var selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(2);
		...
		this_obj.func_set.area_pos_func = {
			...
			setRatioDgmAreaPos : function(){
				...
				//设置比例图额初始位置
				var selr = this_obj.func_set.el_selector_gen_func.genRatioDgmAreaSelr();
				var top =	this_obj.sys_opt.ring_ratio_dgm.ratio_dgm_con_top_offset;
				//alert(top);
				...
				//区域1设置
				var con_height = this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_1_height;
				var selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(1);
				var style_str = "width:100%;height:"+con_height+"px;";//background-color:#ff0000
				this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
				//区域2设置
				var con_height = this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_2_height;
				var selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(2);
				var style_str = "width:100%;height:"+con_height+"px;";//background-color:#ffff00
				this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
				//区域3设置
				var con_height = this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_3_height;
				var selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(3);
				var style_str = "width:100%;height:"+con_height+"px;";//background-color:#00ffff
				this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
				//区域4设置
				var con_height = this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height;
				var selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(4);
				var style_str = "width:100%;height:"+con_height+"px;";//background-color:#ff00ff
				this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
				//区域5设置
				var con_height = this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_5_height;
				var selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(5);
				var style_str = "width:100%;height:"+con_height+"px;";//background-color:#00ff00
				this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
			},
			//设置人口分布图的位置
			setPoplDistbDgmAreaPos : function(){
				...
				// this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
				//判断人口分布图是否需要缩放
				if(this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height < this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_org_height){
					//计算比例
					var scale = parseInt(this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_height)/this_obj.sys_opt.ring_ratio_dgm.rdr_sub_area_4_org_height;

					var selr = this_obj.func_set.el_selector_gen_func.genPoplDistbDgmAreaSelr();
					var style_str = "-webkit-transform:scale("+scale+");transform-origin:50% top;";
					this_obj.func_set.area_pos_func.setElStyle(selr,style_str);
					//对每一行进行缩放
					// var selr = this_obj.func_set.el_selector_gen_func.genPoolPuplDisbItemLineSelr();
					// var style_str = "-webkit-transform:scaleY("+scale+");";
					// $(selr).each(function(index, el) {
					// 	var i_selr = this_obj.func_set.el_selector_gen_func.implSpSelItemSelr(selr,index);
					// 	this_obj.func_set.area_pos_func.setElStyle(i_selr,style_str);	
					// });

					// //对每一个子块进行缩放
					// var selr = this_obj.func_set.el_selector_gen_func.genPoolPuplDisbItemSelr();
					// var style_str = "-webkit-transform:scaleX("+scale+");";
					// $(selr).each(function(index, el) {
					// 	var item_selr = this_obj.func_set.el_selector_gen_func.genPoolPuplDisbSpItemSelr(index);
					// 	//this_obj.func_set.area_pos_func.setElStyle(i_selr,style_str);	
					// 	//图片和信息部分的占比
					// 	var img_width = 20*scale;
					// 	var cnt_width = 100 - img_width;
					// 	var i_selr = this_obj.func_set.el_selector_gen_func.genPPDISubItemRSelr();
					// 	var sub_selr = item_selr + " " + this_obj.func_set.el_selector_gen_func.implSpSelItemSelr(i_selr, 0);//alert();
					// 	var style_str = "width:"+img_width+"%";
					// 	this_obj.func_set.area_pos_func.setElStyle(sub_selr,style_str);	
					// 	var sub_selr = item_selr + " " + this_obj.func_set.el_selector_gen_func.implSpSelItemSelr(i_selr, 1);
					// 	var style_str = "width:"+cnt_width+"%;-webkit-transform:scale("+scale+");transform-origin:left top;";
					// 	this_obj.func_set.area_pos_func.setElStyle(sub_selr,style_str);	

					// 	// //标题
					// 	// var sub_i_selr = sub_selr + " .ci_title";
					// 	// var style_str = "-webkit-transform:scale("+scale+");";
					// 	// this_obj.func_set.area_pos_func.setElStyle(sub_i_selr,style_str);	
					// 	// //内容
					// 	// var sub_i_selr = sub_selr + " .ci_cnt";
					// 	// var style_str = "-webkit-transform:scale("+scale+");";
					// 	// this_obj.func_set.area_pos_func.setElStyle(sub_i_selr,style_str);		
					// 	// //值
					// 	// var sub_i_selr = sub_selr + " .ci_cnt .val";
					// 	// var style_str = "-webkit-transform:scale("+scale+");";
					// 	// this_obj.func_set.area_pos_func.setElStyle(sub_i_selr,style_str);	
					// });
				}
			...
			setHelpPoorIntroAreaPos : function(){
				...
				$(selr).each(function(index, el) {
					...
					//设置子区域的高度
					//区域1设置
					var sub_con_height = this_obj.sys_opt.help_poo_intro_area.hp_sub_area_1_height;
					var sub_selr = this_obj.func_set.el_selector_gen_func.genHIPASubItemSelr(i_selr,1);
					var i_style_str = "width:100%;height:"+sub_con_height+"px;";//background-color:#ff0000
					this_obj.func_set.area_pos_func.setElStyle(sub_selr,i_style_str);
					//区域2设置
					var sub_con_height = this_obj.sys_opt.help_poo_intro_area.hp_sub_area_2_height;
					var sub_selr = this_obj.func_set.el_selector_gen_func.genHIPASubItemSelr(i_selr,2);
					var i_style_str = "height:"+sub_con_height+"px;";//background-color:#ff0000
					this_obj.func_set.area_pos_func.setElStyle(sub_selr,i_style_str);
					//区域3设置
					var sub_con_height = this_obj.sys_opt.help_poo_intro_area.hp_sub_area_3_height;
					var sub_selr = this_obj.func_set.el_selector_gen_func.genHIPASubItemSelr(i_selr,3);
					var i_style_str = "width:100%;height:"+sub_con_height+"px;";//background-color:#ff0000
					this_obj.func_set.area_pos_func.setElStyle(sub_selr,i_style_str);
			...
			pageSwipeLeft : function(){
				...
				//展开标志标志位
				this_obj.sys_opt.page_area.menu_list_unfold_if = false;

				//移动导航栏
				this_obj.func_set.area_pos_func.navSwipeLeft();
			},
			...
			pageSwipeRight : function(){
				...
				this_obj.func_set.area_pos_func.pageHSwipt(h_dis);
				//展开标志标志位
				this_obj.sys_opt.page_area.menu_list_unfold_if = true;
				//移动导航栏
				this_obj.func_set.area_pos_func.navSwipeRight();
			},
			//导航栏左移
			navSwipeLeft:function(){
				var selr = this_obj.func_set.el_selector_gen_func.genNavSelr();
				var h_dis = 0;
				this_obj.func_set.area_pos_func.setElStyle(selr,"left:"+h_dis+"px;");

			},
			//导航栏右移
			navSwipeRight:function(){
				var selr = this_obj.func_set.el_selector_gen_func.genNavSelr();
				var h_dis = this_obj.sys_opt.swipe_h_distance;
				this_obj.func_set.area_pos_func.setElStyle(selr,"left:"+h_dis+"px;");
			},
			...
			ratioDgmSlideToTop : function(){
				...
				this_obj.sys_opt.page_area.ratio_digm_unfold_if = true;
			},
			//比例图向下滑动到底部
			ratioDgmSlideToBottom : function(){
				...
				this_obj.sys_opt.page_area.ratio_digm_unfold_if = false;
			},
		...
		this_obj.func_set.draw_func = {
			...
			ringRatioDgmDrawOk	: function(){
				//下拉比例图,如果比例图已经被用户上拉,则不再关闭
				if(!this_obj.sys_opt.page_area.ratio_digm_unfold_if)
			...
			drawRrdStateCircle : function(){
	            ...
	            var top = y-radius/2;
	            var left = x-radius/2;
	            var p_num = item_obj.num+"人";
	            var num_rt = Math.round(parseInt(item_obj.ratio)/parseInt(this_obj.sys_opt.rd_ring_draw_uinit_count)*1000)/10;
	            //num_rt = "占"+num_rt+"%";
	            var font_size = "font-size:9px;";
	            //屏幕限制
	            if(this_obj.sys_opt.rd_ring_con_height < this_obj.sys_opt.ring_ratio_dgm.rdr_min_screen_limit){
	            	font_size+="-webkit-transform:scale(0.8);";
	            	//alert("rdr_min_screen_limit");
	            }
	            var el_html = '\
	            		<div style="position:absolute;top:'+top+'px;left:'+left+'px;color:#ffffff;'+font_size+'">\
	            			<div  style="text-align: center;color:'+item_obj.color+';">'+item_obj.name+'</div>\
	            			<div style="text-align: center;">'+p_num+'</div>\
	            			<div style="text-align: center;">占<span style="color:#7E3478;font-size:12px;">'+num_rt+'</span>%</div>\
	            		</div>';
	           	//var con_selr = this_obj.func_set.el_selector_gen_func.genRatioDgmAreaSelr();
	           	var con_selr = this_obj.func_set.el_selector_gen_func.genRDgmSubAreaSelr(2);
	           	$(con_selr).append(el_html);
	         //    //填充名称
		        // cans.moveTo(x,y);  
	         //    cans.font = "12px 黑体";  
		        ...
		        //绘制标记圆
		        //半径需要减去描述项圆的半径
				var i_radius = this_obj.sys_opt.ring_ratio_dgm.rdr_state_crle_radius-this_obj.sys_opt.ring_ratio_dgm.rdr_st_item_clr_radius;	
		...
		this_obj.func_set.event_bind_func = {
			...
			bindEvnet : function(){
				...
				this_obj.func_set.event_bind_func.bindShowMenuListBtnClick();
				this_obj.func_set.event_bind_func.bindShowNextPageClick();
			...
			//绑定菜单列表按钮
			bindShowMenuListBtnClick : function(){
				var sub_item_sler = this_obj.func_set.el_selector_gen_func.genShowMenuListBtnSelr();
				$(sub_item_sler).click(function(event) {
					/* Act on the event */
					//alert("ring");
					this_obj.func_set.handle_event_func.handleShowMenuListBtnClick();
				});
			},
			//绑定地图块点击事件
			...
			//绑定点击下一页按钮
			bindShowNextPageClick : function(){
				var selr = this_obj.func_set.el_selector_gen_func.genShowNextPageSelr();//alert(selr);
				$(selr).click(function(event) {
					/* Act on the event */
					//alert("ok");
					this_obj.func_set.handle_event_func.handleShowNextPageClick(this);
			...
			bindRationDgmDropRingClick : function(){
				var sub_item_sler = this_obj.func_set.el_selector_gen_func.genDropDownRingSelr();
				
				$(sub_item_sler).click(function(event) {
					/* Act on the event */
					//alert("od");
					this_obj.func_set.handle_event_func.handleDropRingClick(this);
					//处理时间
		...
		this_obj.func_set.handle_event_func = {
			...
			//处理显示菜单列表按钮点击事件
			handleShowMenuListBtnClick : function(i_this){
				//判断展开情况
				if(this_obj.sys_opt.page_area.menu_list_unfold_if){
					//展开,则收起
					this_obj.func_set.area_pos_func.pageSwipeLeft();
				}
				else
			...
			handleFightMapClick : function(i_this){
				//var sub_item_sler = this_obj.func_set.el_selector_gen_func.genFightMapItemSelr();
				// alert(sub_item_sler + ":eq("+i_this+")");
				 // alert(i_this);
				 // alert($(sub_item_sler + ":eq("+i_this+")").html());
				 // alert($(sub_item_sler + ":eq("+i_this+") [font-size=10]").html());
				 // return;
				//获取编号
				//var divs_num = $(i_this).find("[font-size=10]").html();
				var divs_num = $(i_this).text();
				//alert(divs_num);
				divs_num = divs_num.replace(/(^\s*)|(\s*$)/g, ""); 
				//alert(divs_num);//return;
				//进行跳转
			...
			//处理点击显示下一页事件
			handleShowNextPageClick : function(i_this){
					//获取下一页的选择器
					var next_selr = $(i_this).attr("data-next");//;alert(next_selr);
					$(next_selr).animate({top:"0px"},"slow");
					//var style_str = "top:"+0+"px;";
					//this_obj.func_set.area_pos_func.setElStyle(next_selr,style_str);
			},
			//处理下拉环点击时间
			handleDropRingClick : function(i_this){
					//获取下一页的选择器
					//var next_selr = $(i_this).attr("data-next");//;alert(next_selr);
					//$(next_selr).animate({top:"0px"},"slow");
					//var style_str = "top:"+0+"px;";
					//this_obj.func_set.area_pos_func.setElStyle(next_selr,style_str);
					$(i_this).parent().animate({top:this_obj.sys_opt.dis_area_height+"px"},"slow");
			},
		...
		this_obj.func_set.el_selector_gen_func = {
			...
			//生成导航栏选择器
			genNavSelr : function(){
				return this_obj.func_set.el_selector_gen_func.genMapConSelr() + " .i_header";
			},
			//生成显示菜单区域按钮选择器
			genShowMenuListBtnSelr : function(){
				return this_obj.func_set.el_selector_gen_func.genMapConSelr() + " .i_header .menu_list";
			},
			//生成地图区域的选择器
			...
			//生成比例图子区域选择器
			genRDgmSubAreaSelr : function(index){
				return this_obj.func_set.el_selector_gen_func.genRatioDgmAreaSelr() + " .rd_area_" + index;
			},
			//生成比例图下拉拉环区域选择器
			...
			//生成贫困人口分布项数据行选择器
			genPoolPuplDisbItemLineSelr : function(){
				return this_obj.func_set.el_selector_gen_func.genPoplDistbDgmAreaSelr() + " .pdd_item_con";
			},
			//生成贫困人口分布项选择器
			genPoolPuplDisbItemSelr : function(){
				return this_obj.func_set.el_selector_gen_func.genPoplDistbDgmAreaSelr() + " .pdd_item_con .pdd_data_item";
			},
			//生成贫困人口分布项子元素的相对选择器
			genPPDISubItemRSelr : function(){
				return ".pd_item_ctnt .cnt_item";
			},

			//生成贫困人口分布指定项选择器
			genPoolPuplDisbSpItemSelr : function(index){
				return this_obj.func_set.el_selector_gen_func.genPoolPuplDisbItemSelr() + ":eq("+index+")";
			},
			//生成扶贫概况引导按钮区域选择器
			...
			genHelpPoorIntroAreaSelr : function(){
				return this_obj.func_set.el_selector_gen_func.genMapConSelr() + " .help_intro_area";
			},
			//生成扶贫区域子区域的选择器,指定父选择器
			genHIPASubItemSelr : function(p_selr, index){
				return p_selr + " .hp_area_" + index;
			},
			...
			//生成显示下一页的选择器
			genShowNextPageSelr :function(){
				return ".show_next_page";
			},
			//生成下拉环选择器
			genDropDownRingSelr : function(){
				return ".ratio_dgm_pring_down_area";
			},	


			//合成选择项选择器,指定选择项编号
			implSpSelItemSelr : function(sler, index){
				return sler + ":eq("+index+")";
			},
