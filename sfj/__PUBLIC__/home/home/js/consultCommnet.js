...
var ConsultComment = {	//初始化对象
	...
	initCtrl:function(i_options){
		...
		this_obj.func_set.el_dom_opert_func = {
			...
			setRadioCtrlBeenSel : function(index){
				...
				//设置输入控件的值
				var val_selr = this_obj.func_set.el_selector_gen_func.genRadioCtrlItemValSelr(selr);
				var val = $(val_selr).html();
				//alert(val);
				//填写输入控件值
				var input_selr = this_obj.func_set.el_selector_gen_func.genInputCtrlSelr();
				$(input_selr).val(val);
		...
		this_obj.func_set.el_selector_gen_func = {
			...
			//生成单选框值的选择器
			genRadioCtrlItemValSelr : function(p_selr){
				return p_selr + " .real_data";
			},
			//生成表单控件的选择器
			genInputCtrlSelr : function(p_selr){
				return this_obj.func_set.el_selector_gen_func.genDisAreaSelr() + " form .i_deg_id";
			},
		...
		//调用初始化,并返回对象
		ConsultComment.action_obj = this_obj;
	...
	animateDraw : function(){
		//alert("ok");
		ConsultComment.action_obj.func_set.draw_func.animateDrawRingDgmAction();
	},
	action_obj : "",
